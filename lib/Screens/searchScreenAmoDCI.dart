...
class SearchAmoScreenDCI extends StatefulWidget {
  ...
  SearchAmoScreenDCI({super.key, required this.listes, required this.hintText});

  @override
  State<SearchAmoScreenDCI> createState() => _SearchAmoScreenDCIState();
}

class _SearchAmoScreenDCIState extends State<SearchAmoScreenDCI> {
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        filtered = widget.listes.where((med) {
          final medDCILower = removeDiacritics(med.dci.join(' ').toLowerCase());
          final queryLower = removeDiacritics(query.toLowerCase());
          return medDCILower.contains(queryLower);
        }).toList();
      }
    });
  }
        ...
        child: Column(
          ...
          children: [
            ...
            if (searchController.text.isNotEmpty) ...[
              const Gap(8),
              Text(
                'Essayez avec d\'autres termes D.C.I',
                style: TextStyle(
                  color: Colors.grey[500],
                  fontSize: 14,
                ),
